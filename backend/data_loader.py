...
def load_data():
    ...
    try:
        # Ana predictions dosyası - Final versiyonu kullan (2018-2024 tam veri)
        if "predictions" in data_files:
            # Final dosyasını kullan (2018-2024 tam veri)
            pred_df = pd.read_parquet(data_files["predictions"])
            log_info(f"📊 Final dosyası yüklendi (2018-2024): {data_files['predictions']}")
            
            # Optuna dosyası varsa ek olarak yükle (sadece 2024 tahminleri için)
            optuna_file = data_files["predictions"].replace("final", "optuna")
            if os.path.exists(optuna_file):
                optuna_df = pd.read_parquet(optuna_file)
                log_info(f"🎯 Optuna dosyası da yüklendi (2024 tahminleri): {optuna_file}")
                # Optuna tahminlerini final verisine ekle
                pred_df = pd.concat([pred_df, optuna_df], ignore_index=True)
                pred_df = pred_df.drop_duplicates(subset=['lake_id', 'date', 'H'], keep='last')

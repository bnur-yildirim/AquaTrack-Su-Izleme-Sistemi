...
def horizon_comparison():
    ...
    try:
        ...
        # Unified metrics dosyasından horizon verilerini al
        unified_metrics_path = os.path.join(BACKEND_MODELS_DIR, 'unified_normalized_metrics.json')
        ...
        for horizon in ['H1', 'H2', 'H3']:
            horizon_data = unified_metrics.get(horizon, {}).get(str(lake_numeric_id), {})
            
            if horizon_data:
                comparison.append({
                    'horizon': horizon,
                    'horizon_name': f'{horizon} ({horizon[1]} Ay Öncesi)',
                    'r2': horizon_data.get('r2', 0),
                    'wmape': horizon_data.get('wmape', 0),
                    'unified_score': horizon_data.get('unified_score', 0),
                    'reliability': horizon_data.get('reliability', 'Unknown'),
                    'samples': horizon_data.get('samples', 0)
                })

...
def all_lakes_summary():
    ...
    try:
        ...
        if not os.path.exists(csv_path):
            return jsonify({"error": f"Veri dosyası bulunamadı: {csv_path}"}), 404
        ...
        for lake_id in df['lake_id'].unique():
            ...
            lakes_summary.append({
                "lake_id": int(lake_id),
                "lake_name": lake_data['lake_name'].iloc[0] if 'lake_name' in lake_data.columns else f"Lake {lake_id}",
                "measurements": len(lake_data),
                "dominant_cluster": dominant_cluster,
                "cluster_name": dominant_info["name"],
                "risk_level": dominant_info["risk_level"],
                "icon": dominant_info["icon"]
            })

...
export default function WaterQuality({ selectedLake = 'van' }) {
  ...
  // Timeline için tüm kayıtlar
  const timelineData = lakeData.history?.map(record => ({
    ...
    cluster: record.cluster
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 16px' }}>
        ...
        {/* ZAMAN SERİSİ - LİNE CHART */}
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: '700', color: '#1e293b', marginBottom: 20 }}>
            📈 Su Kalitesi Parametreleri - Zaman Serisi
          </h2>
          
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={timelineData.slice(-100)} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                        <XAxis 
                          dataKey="date" 
                          angle={-45} 
              ...
              <Line type="monotone" dataKey="chl_a" stroke="#10b981" strokeWidth={2} dot={false} name="Chl-a" />
            ...
            Son 100 ölçüm gösteriliyor • Toplam: {timelineData.length} kayıt
                  </div>
                </div>

...
export default function AnalyticsPage() {
  ...
  const fetchSummary = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/metrics/unified/summary`);
  ...
  const fetchMatrixData = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/quality/matrix-analysis`);
      const result = await response.json();
      if (result.status === 'success') {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-slate-100">
                ...
                <div key={horizon} className="relative group">
                  ...
                  <div className="relative bg-slate-800 p-6 rounded-2xl border border-slate-600 shadow-lg">
                    ...
                    <div className="text-4xl font-black text-slate-100 mb-3">
                      {(data.avg_score * 100).toFixed(1)}<span className="text-2xl text-slate-300">%</span>
                    </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* 3D Analysis Tab - YENİ! */}
        {activeTab === '3d' && (
          <div className="space-y-8">
            {!matrixLoading && matrixData ? (
              <WaterQuality3DScatter matrixData={matrixData} />
            ) : (
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-lg p-12">
                <div className="text-center">
                  <div className="w-16 h-16 border-4 border-slate-200 border-t-slate-600 rounded-full animate-spin mx-auto mb-4"></div>
                  <p className="text-slate-600 text-lg">3D Analiz Yükleniyor...</p>
                </div>
              </div>
            )}
            
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-lg p-8">
              ...
              <div className="text-slate-600 space-y-3 leading-relaxed">
                <p>
                  <strong className="text-slate-800">3D Scatter Plot:</strong> Tüm göllerin tüm yıllarını 
                  3 boyutlu uzayda görselleştirir.
                </p>
                <ul className="list-disc list-inside space-y-2 ml-4">
                  <li><strong>X Eksen:</strong> NDWI (Normalized Difference Water Index) - Su varlığı göstergesi</li>
                  <li><strong>Y Eksen:</strong> Chlorophyll-a - Alg yoğunluğu ve su kalitesi</li>
                  <li><strong>Z Eksen:</strong> Turbidity - Bulanıklık seviyesi</li>
                  <li><strong>Renk:</strong> Cluster türü (4 farklı su kalitesi grubu)</li>
                  <li><strong>Boyut:</strong> Ölçüm sayısı (daha büyük = daha fazla veri)</li>
                </ul>
                <p className="mt-4 p-4 bg-slate-100 rounded-lg border border-slate-200">
                  <strong>💡 İpucu:</strong> Fareyle sürükleyerek grafiği döndürebilir, scroll ile yakınlaştırabilirsiniz. 
                  Otomatik döndürme özelliğini kullanarak tüm açılardan görüntüleyebilirsiniz.
                </p>
              </div>
            </div>
          </div>
...
function DataQualityAnalysis() {
  ...
  useEffect(() => {
    // API'den veri çek
    fetch(`${API_BASE}/api/analytics/lake-performance`)
      .then(res => res.json())
      .then(result => {
        if (result.status === 'success') {
          setData(result.data);
...
function FeatureImportanceChart() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/analytics/feature-importance?horizon=${horizon}`)
      .then(res => res.json())
      .then(result => {
        if (result.status === 'success') {

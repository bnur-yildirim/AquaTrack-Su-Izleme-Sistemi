  ...
  return (
    <div style={{ 
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '24px 16px' }}>
        ...
        {/* CIRCLE PACKING - Su Miktarı Hiyerarşik Görünüm (EN ÜST) */}
        ...
        {!metricsLoading && metrics && metrics.data && (
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '24px' }}>
            <div style={{ maxWidth: '350px', width: '100%' }}>
              <PerformanceCard
                percentage={metrics.data.H1 ? (100 - metrics.data.H1.avg_wmape).toFixed(1) : '95.0'}
                mape={metrics.data.H1 ? metrics.data.H1.avg_wmape : 5.0}
                onClick={() => {}}
              />
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <h2 style={{ 
                ...
                {loading ? '...' : allLakesData?.length || 7} Göl Karşılaştırması
              </h2>
              <p style={{ 
                margin: 0,
                color: COLORS.medium,
                ...
                Model başarı oranları ve su alanı verileri
              </p>
            </div>
          </div>
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: '30px' }}>
              ...
              <p style={{ marginTop: '12px', color: COLORS.medium, fontSize: '13px' }}>Veriler yükleniyor...</p>
            </div>
          ) : (
            <LakeComparisonChart allLakesData={allLakesData} />
          )}
        </div>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  {/* Grouped Bar Chart - Yıllık Karşılaştırma */}
                  <div style={{ marginBottom: 24 }}>
                    <h3 style={{
                      color: COLORS.dark,
                      fontSize: '1.125rem',
                      ...
                      📊 Yıllara Göre Su Alanı Karşılaştırması
                    </h3>
                    <ResponsiveContainer width="100%" height={400}>
                      <BarChart data={yearlyData} margin={{ top: 20, right: 30, left: 60, bottom: 20 }}>
                        <defs>
                          {allLakesData.map((lake, idx) => (
                            <linearGradient key={idx} id={`bar-gradient-${idx}`} x1="0" y1="0" x2="0" y2="1">
                              <stop offset="0%" stopColor={['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#06b6d4', '#ec4899'][idx % 7]} stopOpacity={0.9}/>
                              <stop offset="100%" stopColor={['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#06b6d4', '#ec4899'][idx % 7]} stopOpacity={0.5}/>
                            </linearGradient>
                          ))}
                        </defs>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
        ...
        {/* YENİ: 3D Heatmap - Göller × Yıllar × Su Alanı */}
        {!loading && allLakesData && allLakesData.length > 0 && (
